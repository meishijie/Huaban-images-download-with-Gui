...
def get_huaban_beauty(pid):
    ...
    while board_id != None:
        ...
        try:
            ...
            if len(groups) <= 0:
                action.configure(text=name.get() + '开始下载！')  # 设置button显示的内容
...
def downloadall(alldata):
    for att in alldata:
        ...
        if urllib.request.urlretrieve(img_url, GPATH + '/' + board_id + '/' + att_url + '.' + img_type):
            ...
            downloadedCount += 1
            global downloaded
            downloaded.set('下载了：'+str(downloadedCount)+'/'+str(len(alldata)))
            WIN.update()
        else:
            print (img_url + '.' + img_type + ' save failed')
    action.configure(text=name.get() + '下载成功！')  # 设置button显示的内容
    action.configure(state='normal')
